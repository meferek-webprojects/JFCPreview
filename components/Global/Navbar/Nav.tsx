...
const Nav = ({ main }: Props) => {
    ...
    useEffect(() => {
        const handleScroll = () => {
            if(window.pageYOffset <= 0){
                ...
                if((!navOpen && windowSize < 1280) && (main || (!main && window.pageYOffset > 88))){
                    if(prevOffset.current > window.pageYOffset){
                        setNavHidden(false)
                    }else if(prevOffset.current < window.pageYOffset){
                        setNavHidden(true)
                    }
                }else {
                    setNavHidden(false)
    ...
    return (
        <>
            <ShopIcon offset={offset}/>
            <ScrollToTop offset={offset} setOffset={setOffset}/>
            <div className={"xl:py-3 flex flex-wrap z-30 top-0 w-full transition ease-in-out duration-300" + ((!main || offset >= 10) ? " bg-blue-700" : "") + (main ? " fixed" : " sticky") + (navHidden ? " -translate-y-full" : "")}>
                <div className={"flex w-full z-30 xl:hidden transition duration-200" + ((navOpen || !main) ? " bg-blue-700" : "") + (navOpen ? "" : " delay-[250ms]")}>
                    <NavLink link="/" noHover="true" className="mr-auto ml-3"><Image priority={true} alt="image" src={ logoMiniImage } width="75px" height="50px"/></NavLink>
                    <div className="flex px-3 mr-2 text-white hover:scale-[1.2] transition ease-in-out duration-300" onClick={() => { setNavOpen(!navOpen) }}>
                        { navOpen ?
